...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['place_order'])) {
    ...
    if ($stock_ok) {
        ...
        try {
            ...
            // Store order ID + payment method in session (used by cust_orders.php later)
            $_SESSION['last_order_id']     = $order_id;
            $_SESSION['last_pay_method']   = $pay_method;
            $_SESSION['last_order_total']  = $total;

            // Build the receipt data to display in a popup on THIS page,
            ...
            $receipt_data = [
                ...
                'pay_method'   => ($pay_method === 'qr') ? 'QR Payment' : 'Card Payment',
                'ship_zone'    => ($ship_zone === 'borneo') ? 'Sabah / Sarawak' : 'Semenanjung Malaysia',
...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Englebert', sans-serif; }
        ...
        /* Receipt Modal */
        .receipt-modal { background-color: #fff; padding: 0; border-radius: 18px; text-align: left; max-width: 420px; width: 100%; max-height: 88vh; overflow-y: auto; box-shadow: 0 10px 25px rgba(0,0,0,0.3); }
        .receipt-header { background-color: var(--primary-blue); color: #fff; padding: 22px 24px 18px; text-align: center; border-radius: 18px 18px 0 0; }
        .receipt-header i { font-size: 38px; color: #4ade80; margin-bottom: 8px; display: block; }
        .receipt-header h3 { font-size: 22px; margin-bottom: 4px; }
        .receipt-header p { font-size: 13px; color: #cbd5e1; }
        .receipt-body { padding: 20px 24px; }
        .receipt-section { margin-bottom: 16px; }
        .receipt-section-title { font-size: 12px; text-transform: uppercase; letter-spacing: 0.5px; color: #888; font-weight: bold; margin-bottom: 6px; }
        .receipt-row { display: flex; justify-content: space-between; font-size: 14px; color: #333; margin-bottom: 4px; }
        .receipt-divider { border-top: 1px dashed #ccc; margin: 14px 0; }
        .receipt-item { display: flex; justify-content: space-between; font-size: 14px; color: #1A1A1A; margin-bottom: 6px; }
        .receipt-item span:first-child { flex: 1; padding-right: 8px; }
        .receipt-total-row { display: flex; justify-content: space-between; font-size: 19px; font-weight: bold; color: var(--primary-blue); padding-top: 10px; border-top: 2px solid var(--primary-blue); margin-top: 8px; }
        .receipt-discount { color: #16a34a; }
        .receipt-footer { padding: 0 24px 22px; }
        .receipt-close-btn { background-color: var(--primary-blue); color: white; border: none; padding: 12px; font-size: 16px; border-radius: 10px; cursor: pointer; font-weight: bold; width: 100%; font-family: 'Englebert', sans-serif; }
        .receipt-close-btn:hover { background-color: #071c35; }
        .receipt-id-badge { display: inline-block; background: rgba(255,255,255,0.15); padding: 4px 12px; border-radius: 20px; font-size: 13px; margin-top: 6px; }
...
<div id="receiptModalOverlay" class="modal-overlay active">
    <div class="receipt-modal">

        <div class="receipt-header">
            <i class="fas fa-circle-check"></i>
            <h3>Order Placed Successfully!</h3>
            <p>Thank you for shopping with Dreambound Bookstore</p>
            <div class="receipt-id-badge">Order #<?php echo htmlspecialchars($receipt_data['order_id']); ?></div>
        </div>
        ...
        <div class="receipt-body">
            ...
            <div class="receipt-section">
                ...
                <div class="receipt-row"><span>Date</span><span><?php echo htmlspecialchars($receipt_data['date']); ?></span></div>
                <div class="receipt-row"><span>Payment Method</span><span><?php echo htmlspecialchars($receipt_data['pay_method']); ?></span></div>
                <div class="receipt-row"><span>Shipping Zone</span><span><?php echo htmlspecialchars($receipt_data['ship_zone']); ?></span></div>
            </div>

            <div class="receipt-section">
                <div class="receipt-section-title">Delivered To</div>
                <div class="receipt-row"><span>Name</span><span><?php echo htmlspecialchars($receipt_data['customer']); ?></span></div>
                <div class="receipt-row"><span>Email</span><span><?php echo htmlspecialchars($receipt_data['email']); ?></span></div>
                <div class="receipt-row"><span>Phone</span><span><?php echo htmlspecialchars($receipt_data['phone']); ?></span></div>
                <div class="receipt-row"><span>Address</span><span style="text-align:right; max-width:230px;"><?php echo htmlspecialchars($receipt_data['address']); ?></span></div>
            </div>
            ...
            <div class="receipt-section">
                ...
                <div class="receipt-item">
                    <span><?php echo htmlspecialchars($item['title']); ?> &times; <?php echo (int)$item['qty']; ?></span>
            ...
            <div class="receipt-row"><span>Subtotal</span><span>RM <?php echo number_format($receipt_data['subtotal'], 2); ?></span></div>
            <div class="receipt-row"><span>Shipping Fee</span><span>RM <?php echo number_format($receipt_data['shipping_fee'], 2); ?></span></div>
            <?php if ($receipt_data['discount'] > 0): ?>
            <div class="receipt-row receipt-discount"><span>Voucher (<?php echo htmlspecialchars($receipt_data['voucher_code']); ?>)</span><span>- RM <?php echo number_format($receipt_data['discount'], 2); ?></span></div>
            <?php endif; ?>

            <div class="receipt-total-row">
                <span>Total Paid</span>
                <span>RM <?php echo number_format($receipt_data['total'], 2); ?></span>
            </div>

        </div>

        <div class="receipt-footer">
            <button class="receipt-close-btn" onclick="window.location.href='cust_orders.php?order_success=<?php echo $receipt_data['order_id']; ?>'">
                <i class="fas fa-list-check"></i> Continue to My Orders
            </button>
        </div>
